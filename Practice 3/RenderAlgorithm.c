...
void clean(void){                                                               //Function to reset PPM File

    int i; int j;

    for(i = MAXN-1; i  >= 0; i--){
        for(j = 0; j < MAXN; j++){
            matrixToDraw[j][i].red = 0;
            matrixToDraw[j][i].green = 0;
            matrixToDraw[j][i].blue = 0;
        }
    }
}
...
void goesToMagic(int ctFrame){                                                   //Function to draw in the .ppm File
    drawFigure(matrixToDraw, ctFrame);
}